...
# Local
from utils_valorant.useful import _extract_tokens, extract_tokens_from_url
from utils_valorant.json_loader import data_read, data_save, config_read
...
class Auth:
    ...
    def get_users(self):
        try:
            ...
            raise commands.UserInputError(f"you're not registered!, plz `/login` to register!")
    ...
    def authenticate(self):
        ...
        if r.json()['type'] == 'response':
            ...
            if r.status_code == 429:
                raise commands.UserInputError('Please wait a few minutes and try again.')
            
            database[self.user_id] = cookies
            database[self.user_id]['WaitFor2FA'] = int(datetime.timestamp(datetime.now()))
            data_save('users', database)
            if r.json()['multifactor']['method'] == 'email':
                raise commands.UserInputError(f"**Riot sent a code to {r.json()['multifactor']['email']}** `/2fa` to complete your login.")
            raise commands.UserInputError('**You have 2FA enabled!** use `/2fa` to enter your code.')
    ...
    def get_entitlements_token(self):
        ...
        try:
            ...
            raise commands.UserInputError(f'Cookies is expired, plz /login again!')
    ...
    def get_userinfo(self):
        ...
        try:
            ...
            raise commands.UserInputError(f'An unknown error occurred, plz /login again')
    ...
    def temp_auth(self):
        ...
        if r.json()['type'] == 'response':
            ...
            raise commands.UserInputError("if you're not registered! is not support 2FA, plz `/login` first")
        else:
            raise commands.UserInputError('Your username or password may be incorrect!')
