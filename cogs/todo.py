...
class Todo(commands.Cog):
    ...
    async def todo_list(self, ctx):
        ...
        for x in data:
            number = number + 1
            todo_entries = f"**[{x['todo_id']}]({x['jump_url']})**. {x['content']} ({format_dt(x['creation_date'], style='R')})"
    ...
    async def todo_remove(self, ctx, number: int = commands.Option(description="Todo number")):
        ...
        data = await self.bot.latte_todo.find_by_custom({"user_id": ctx.author.id, "todo_id": int(number)})
        #check_data
        if bool(data) == False:
            embed_error.description = "Todo not found"
            return await ctx.send(embeb=embed_error, ephemeral=True, delete_after=15)

        data_deleting = await self.bot.latte_todo.delete_by_custom({"user_id": ctx.author.id, "todo_id": int(number)})
        ...
        if data_deleting and data_deleting.acknowledged:
            ...
            embed.description =f"{data['content']} ({format_dt(data['creation_date'], style='R')})"
    ...
    @commands.command(aliases=['tdc','todoc','todoclear'], help="Deletes all tasks from your todo list.")
    @is_latte_guild()
    ...
    async def todo_edit(
        ...
        old = await self.bot.latte_todo.find_by_custom({"user_id": ctx.author.id, "todo_id": number})
